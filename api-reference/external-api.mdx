...
- `status` – (string, optional) Filter by action status (`initiated`, `executed`, `failed`)
- `type` – (string, optional) Filter by action type
- `created_at_from`, `created_at_to` – (ISO date, optional) Filter by creation time range
...
- `page` – (integer) Page number (default: 1)
- `limit` – (integer) Number of results per page (default: 20, max: 100)
- `role` – (string, optional) Filter messages by role (`user`, `assistant`)
- `created_at_from`, `created_at_to` – (ISO date, optional) Filter by message creation time ranges
